...
struct HistoryView: View {
    ...
    init(context: NSManagedObjectContext) {
        _historyViewModel = ObservedObject(wrappedValue: HistoryViewModel(context: context))
    }
        ...
        NavigationStack {
            VStack(spacing: 0) {
                ...
                if historyViewModel.chatHistory.isEmpty {
                    ...
                    List {
                        ForEach(historyViewModel.chatHistory, id: \.self) { sectionItem in
                            Section(header: headerView(for: sectionItem)) {
                                ForEach(sectionItem.groupedItemByThread.reversed(), id: \.self) { rowItem in
                                    NavigationLink(destination: ThreadView(threadData: rowItem)) {
    ...
    @ViewBuilder
    func headerView(for sectionItem: RequiredFormOfData) -> some View {
    ...
    @ViewBuilder
    func cellView(for rowItem: [ChatHistory]) -> some View {
